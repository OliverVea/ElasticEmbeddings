...
public class DocumentIndexingService(
    ...
    IElasticIndexState elasticIndexState,
    ElasticsearchClient elasticsearchClient,
    IMappingDescriptorService<Document> documentMappingDescriptorService) : IDocumentIndexingService
{
    ...
    private List<Document> GetDocumentBatch()
    {
        ...
        {
            if (!elasticIndexState.Documents.TryDequeue(out var document)) break;
    ...
    private async Task EnsureIndexAsync(CancellationToken cancellationToken)
    {
        ...
        var createIndexResponse = await elasticsearchClient.Indices.CreateAsync<Document>(Constants.IndexName, configureRequest =>
        {
            configureRequest.Mappings(typeMappingDescriptor => documentMappingDescriptorService.AddTypeMapping(typeMappingDescriptor));
        }, cancellationToken);
